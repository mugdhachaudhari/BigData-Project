...
(select TRANSFORM(pickup_longitude, pickup_latitude) USING 'python findairport.py' AS (loc) from taxi_trips_y) arpt
group by loc;
 
 drop table if exists airport_y;
 
create table airport_y ROW FORMAT DELIMITED 
FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n'
LOCATION '${OUTPUT}/airport_y' as
