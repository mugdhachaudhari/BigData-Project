...
(select CAST(hour(tpep_pickup_datetime) as int) hr, pickupRgn from taxi_trips_y where pickupRgn <> 'UNKNOWN' ) hr_rgn
group by hr, pickupRgn;


 drop table if exists high_densty_rgn_y; 
 
create table high_densty_rgn_y ROW FORMAT DELIMITED 
FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n'
LOCATION '${OUTPUT}/HighDensityRgn_y' as
